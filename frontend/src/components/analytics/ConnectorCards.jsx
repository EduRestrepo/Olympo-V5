...
const ConnectorCards = ({ bridges, onViewProfile }) => {
    ...
    const hasMore = bridges.length > visibleCount;

    return (
        <div className="connector-cards-section">
            <div className="connectors-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem', marginTop: '1rem' }}>
                {bridges.slice(0, visibleCount).map((bridge, index) => (
            ...
            {hasMore && (
                <div style={{ display: 'flex', justifyContent: 'center', marginTop: '1.5rem' }}>
                    ...
                    >
                        Mostrar más conectores ({bridges.length - visibleCount} restantes)
                    </button>
                </div>
            )}
        </div>
